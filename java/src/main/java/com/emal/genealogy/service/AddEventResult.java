...
public sealed interface AddEventResult {
    ...
    record Success(boolean success, Event event, List<Person> newPersons, String message) implements AddEventResult {
        public Success(Event event, List<Person> newPersons, String message) {
            this(true, event, newPersons, message);
        }
    }
